# Direct copy of `submodules/IB-INN/inn_architecture.py` file to change only line 125 and 131 to also apply to FakeMNIST.
...
def constuct_inn(classifier, verbose=False):
    ...
    if 'mnist' in classifier.dataset.lower(): # Modified from # if classifier.dataset == 'MNIST':
    ...
    for i, (width, n_blocks) in enumerate(zip(conv_widths, n_coupling_blocks_conv)):
        if 'mnist' in classifier.dataset.lower() and i==0:  # Modified from if classifier.dataset == 'MNIST' and i==0:
            continue
